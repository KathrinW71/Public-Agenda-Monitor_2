...
| Source | Initial qualification rule |
|---|---|
| Federal President | All published appointments; the first real vertical slice uses the supplied public appointment calendar. |
| Federal Administrative Court | Case starts and judgment announcements. |
| UN Women | High-level panels, ministerial meetings, and reports on gender equality, violence, care work, and gender data. |
...
- Three narrow, public-source adapters using a fixed page or feed per source and a shared normalized event schema.
...
Build one complete vertical slice first, beginning with the Federal President source. After it produces a candidate that a researcher can approve and an editor can see, add the two other adapters through the same schema.
...
1. All three sources produce normalized candidate events using their explicit rules.
