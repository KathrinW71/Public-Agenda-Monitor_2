...
- **Binds:** Federal President, Federal Constitutional Court, and UN Women retrieval
- **Prevents:** source-specific fields leaking into the agenda or three incompatible implementations
- **Rule:** Every adapter accepts a retrieval request and returns either normalized events or one structured source-health result. It must not write directly to the database. The Federal President adapter is the first complete slice; add the Federal Constitutional Court and UN Women only through the same interface. The Federal President adapter first reads the supplied Terminkalender page; if automated retrieval is blocked or cannot be reliably parsed, it uses the official appointments RSS feed as its fallback. The UN Women adapter first reads the official news and publications pages, then falls back to the supplied official news or publications feed when its respective page cannot be retrieved or reliably parsed.
...
public-agenda-monitor/
  ...
  adapters/
    federal-president/       # first end-to-end official-source adapter
    federal-constitutional-court/ # add after the first slice works
    un-women/                # news and publications; add after the first slice works
  lib/
