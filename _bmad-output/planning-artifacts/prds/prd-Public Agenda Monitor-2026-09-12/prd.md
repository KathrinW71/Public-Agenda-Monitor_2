...
**Description:** A Documentary Researcher can manually retrieve events from each agreed Source. The product first delivers the Federal President Source as a complete vertical slice, then the Federal Constitutional Court and UN Women through the same user experience. The product uses only the fixed, official public pages and feeds specified below and applies transparent qualification rules. It prioritizes recall: a plausible event is presented as a Candidate Event rather than silently excluded.
...
- The MVP supports only these Sources:
  - Federal President: `https://www.bundespraesident.de/SiteGlobals/Forms/Suche/Termine/Terminsuche_Formular.html?nn=222254`
  - Federal Constitutional Court: `https://www.bundesverfassungsgericht.de/DE/Aktuelles/TermineWochenausblick/termine-Wochenausblick_node.html`
  - UN Women news: `https://www.unwomen.org/en/news-stories`; RSS fallback: `http://unwomen.org/en/rss-feeds/news`
  - UN Women publications: `https://www.unwomen.org/en/publications`; RSS fallback: `http://unwomen.org/en/feeds/publications`
- For the Federal President, retrieve the supplied Terminkalender page first. If automated retrieval is blocked or the page cannot be reliably parsed, use the official Federal President appointments RSS feed as the fallback.
- Scheduled scanning is not triggered by the MVP.
- The Federal President Source is demonstrated end-to-end before the Federal Constitutional Court and UN Women Sources are added.
...
- The three named public Sources, delivered in the Federal President-first sequence.
...
- **SM-1:** During the demo, all three Sources produce rule-qualified Candidate Events, or an explicitly labelled Fixture demonstrates an unavailable Source. Validates FR-1, FR-2, FR-14.
