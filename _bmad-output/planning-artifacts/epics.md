...
- FR-1: A Documentary Researcher can manually retrieve each approved official source on demand, delivering the Federal President vertical slice first.
...
- NFR-2: Use only the three approved public sources: Federal President, Federal Constitutional Court, and UN Women (News and Publications channels).
...
- Federal President retrieval is page-first with official RSS fallback; UN Women is page-first for News and Publications with their official RSS fallbacks.
...
### Story 1.1: Erster vollständiger Quellenabruf für den Bundespräsidenten

As a Documentary Researcher,
I want to manually retrieve Federal President appointments into a Candidate Event list,
...
**When** a Documentary Researcher selects the Federal President retrieval action,
**Then** a server-side workflow reads the official appointments page and uses its official RSS fallback only when page retrieval or parsing is unavailable,
**And** qualifying appointments appear as normalized Candidate Events with source, direct source link, title, event type, date, stable identity, and supplied optional facts without guessing.
...
### Story 1.3: Erweiterung auf alle vereinbarten Quellen

As a Documentary Researcher,
I want to retrieve the Federal Constitutional Court and UN Women through the same candidate workflow,
So that the agreed three-source research base is complete.

**Acceptance Criteria:**

**Given** the Federal President vertical slice is working,
**When** the researcher manually retrieves the other approved sources,
**Then** the Federal Constitutional Court and UN Women News and Publications channels return qualifying normalized Candidate Events through the shared adapter interface,
**And** UN Women uses its corresponding official RSS fallback only when its official page cannot be retrieved or reliably parsed.
